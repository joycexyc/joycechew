...
var searchForSong = function() {
    ...
    for (var k = 0; k < results.length; k++) {
      ...
      printableResults += (k + 1) + ". " + song.title + " by " + song.artist + "\n";
    }
 
    var songSearched = prompt(results.length + " songs were found:\n\n" + printableResults + "\nPlease enter the number for the song you would like to add.");
 
    songSearched = parseInt(songSearched, 10) - 1;
 
    var song = results[songSearched];
    ...
    var noMatch = prompt("No songs match your query. Would you like to add a new song? (Y)es or (N)o?");
    noMatch = noMatch.toLowerCase();
        if (noMatch === "y") {
            addSong();
            }
        else {
